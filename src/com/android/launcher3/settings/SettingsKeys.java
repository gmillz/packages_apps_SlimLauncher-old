package com.android.launcher3.settings;

/**
 * Created by gmillz on 9/8/14.
 */
public interface SettingsKeys {

    public static final String KEY_SHOW_SEARCH_BAR = "show_search_bar";
    public static final String KEY_HOMESCREEN_GRID = "homescreen_grid";
    public static final String KEY_DRAWER_GRID = "drawer_grid";
    public static final String KEY_ICON_SIZE = "icon_size";
    public static final String KEY_DOCK_ICONS = "dock_icon_count";
    public static final String KEY_SCROLL_WALLPAPER = "scroll_wallpaper";

    // Gestures
    public static final String UP_GESTURE_ACTION = "up_gesture_action";
    public static final String DOWN_GESTURE_ACTION = "down_gesture_action";
    public static final String PINCH_GESTURE_ACTION = "pinch_gesture_action";
    public static final String SPREAD_GESTURE_ACTION = "spread_gesture_action";
    public static final String DOUBLE_TAP_GESTURE_ACTION = "double_tap_gesture_action";

    // Notification Badges
    public static final String KEY_ENABLE_BADGES = "enable_badges";
    public static final String KEY_BADGE_POSITION = "badge_position";
    public static final String KEY_BADGE_FRAME_SIZE = "badge_frame_size";
    public static final String KEY_BADGE_TEXT_SIZE = "badge_text_size";
    public static final String KEY_BADGE_CORNER_RADIUS = "badge_corner_radius";
    public static final String KEY_BADGE_SIDE_PADDING = "badge_side_padding";
    public static final String KEY_BADGE_TOP_PADDING = "badge_top_padding";
    public static final String KEY_BADGE_BACKGROUND_COLOR = "badge_background_color";
    public static final String KEY_BADGE_TEXT_COLOR = "badge_text_color";
    public static final String KEY_BADGE_FRAME_COLOR = "badge_frame_color";
}
